#ifndef _TRACI_COMMAND_INTERFACE_H_
#define _TRACI_COMMAND_INTERFACE_H_
...
class TraCICommandInterface
{
	public:
		...
		std::pair<double, double> getLonLat(const Point&);
		double getDistance(const Point& position1, const Point& position2, bool returnDrivingDistance);

		// Vehicle methods
		bool addVehicle(std::string vehicleId, std::string vehicleTypeId, std::string routeId, double emitTime_st = -DEPART_NOW, double emitPosition = -DEPART_POS_BASE, double emitSpeed = -DEPART_SPEED_MAX, int8_t emitLane = -DEPART_LANE_BEST_FREE);
		class Vehicle {
			public:
				...
				// std::string getJunctionId();			// ADD cam traffic
				double getSpeed();
				double getAngle();		// ADD cam traffic
				double getAcceleration();
				double getLength();
				double getWidth();
				int    getLights();		// ADD cam traffic
				Point getVehiclePosition();
		...
		void addPolygon(std::string polyId, std::string polyType, const TraCIColor& color, bool filled, int32_t layer, const std::list<Point>& points);
		class Polygon {
			public:
				...
				std::string getTypeId();
				std::list<Point> getShape();
				void setShape(const std::list<Point>& points);
		...
		// Poi methods
		void addPoi(std::string poiId, std::string poiType, const TraCIColor& color, int32_t layer, const Point& pos);
		class Poi {
			public:
		...
		class GuiView {
			public:
				...
				void setBoundary(Point p1, Point p2);
	...
	private:
		...
		Point genericGetCoord(uint8_t commandId, std::string objectId, uint8_t variableId, uint8_t responseId);
		...
		std::list<Point> genericGetCoordList(uint8_t commandId, std::string objectId, uint8_t variableId, uint8_t responseId);
};
