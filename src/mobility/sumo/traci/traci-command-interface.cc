...
int TraCICommandInterface::Vehicle::getLights(){
	return traci->genericGetDouble(CMD_GET_VEHICLE_VARIABLE, nodeId, VAR_SIGNALS, RESPONSE_GET_VEHICLE_VARIABLE);
}

Point TraCICommandInterface::Vehicle::getVehiclePosition(){
...
double TraCICommandInterface::getDistance(const Point& p1, const Point& p2, bool returnDrivingDistance) {
	...
	TraCIBuffer buf = connection.query(CMD_GET_SIM_VARIABLE, TraCIBuffer() << variable << simId << variableType << count << TraCICoord(p1.x,p1.y) << TraCICoord(p2.x,p2.y) << dType);
...
std::list<Point> TraCICommandInterface::Polygon::getShape() {
	return traci->genericGetCoordList(CMD_GET_POLYGON_VARIABLE, polyId, VAR_SHAPE, RESPONSE_GET_POLYGON_VARIABLE);
}

void TraCICommandInterface::Polygon::setShape(const std::list<Point>& points) {
	TraCIBuffer buf;
	...
	for (std::list<Point>::const_iterator i = points.begin(); i != points.end(); ++i) {
		const TraCICoord& pos = TraCICoord(i->x,i->y);
...
void TraCICommandInterface::addPolygon(std::string polyId, std::string polyType, const TraCIColor& color, bool filled, int32_t layer, const std::list<Point>& points) {
	TraCIBuffer p;
...
void TraCICommandInterface::addPoi(std::string poiId, std::string poiType, const TraCIColor& color, int32_t layer, const Point& pos_) {
	TraCIBuffer p;

	TraCICoord pos(pos_.x,pos_.y);
...
std::list<Point> TraCICommandInterface::Lane::getShape() {
...
Point TraCICommandInterface::Junction::getPosition() {
	return traci->genericGetCoord(CMD_GET_JUNCTION_VARIABLE, junctionId, VAR_POSITION, RESPONSE_GET_JUNCTION_VARIABLE);
}

bool TraCICommandInterface::addVehicle(std::string vehicleId, std::string vehicleTypeId, std::string routeId, double emitTime_st, double emitPosition, double emitSpeed, int8_t emitLane) {
	bool success = false;

	uint8_t variableId = ADD;
	uint8_t variableType = TYPE_COMPOUND;
	int32_t count = 6;
	int32_t emitTime = (emitTime_st < 0) ? (-1) : (floor(emitTime_st * 1000));
...
void TraCICommandInterface::GuiView::setBoundary(Point p1_, Point p2_) {
	TraCICoord p1(p1_.x,p1_.y);
	TraCICoord p2(p2_.x,p2_.y);
...
Point TraCICommandInterface::genericGetCoord(uint8_t commandId, std::string objectId, uint8_t variableId, uint8_t responseId) {
...
std::list<Point> TraCICommandInterface::genericGetCoordList(uint8_t commandId, std::string objectId, uint8_t variableId, uint8_t responseId) {
