...
main (int argc, char *argv[])
{
  ...
  //ueStaticRouting->SetDefaultRoute (Ipv4Address ("7.0.0.0"), Ipv4Mask ("255.0.0.0"), 1);
  //ueStaticRouting->AddNetworkRouteTo (Ipv4Address ("7.0.0.0"), Ipv4Mask ("255.0.0.0"), 2);
  ...
  EvalvidClientHelper client (ueIpIface.GetAddress (0),port);
  client.SetAttribute ("ReceiverDumpFilename", StringValue("rd_a01_lte"));
  apps = client.Install (ueNodes.Get(1));
  ...
  //std::cerr<<ueIpIface.GetAddress (0)<<std::endl<<" "<<ueIpIface.GetAddress (1)<<std::endl;
