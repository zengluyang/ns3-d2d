...
void ReceivePacket (Ptr<Socket> socket)
{
  while (socket->Recv ())
    {
      std::cout<<Simulator::Now()<<" RECV "<<socket->GetNode ()->GetId ()<<std::endl;
    }
}
  ...
  if (pktCount > 0)
    {
      ...
      std::cout<<Simulator::Now()<<" SEND "<<socket->GetNode ()->GetId ()<<std::endl;
...
int main (int argc, char *argv[])
{
  ...
  cmd.AddValue ("nodeCount", "nodeCount", nodeCount);
  cmd.Parse (argc, argv);
